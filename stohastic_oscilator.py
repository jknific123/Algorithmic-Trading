...
def stohastic_oscilator(high_low_period, d_sma_period, df, ticker, starting_index, status, odZacetkaAliNe):
    # naredimo nove stolpce za EMA-e, MACD in signal line

    df[f'Low-{high_low_period}-days'] = df['Adj Close'].rolling(window=high_low_period, min_periods=1, center=False).min()
    df[f'High-{high_low_period}-days'] = df['Adj Close'].rolling(window=high_low_period, min_periods=1, center=False).max()
    df['%K'] = (df["Adj Close"] - df[f'Low-{high_low_period}-days']) / (df[f'High-{high_low_period}-days'] - df[f'Low-{high_low_period}-days']) * 100
    df[f'%D-{d_sma_period}-days'] = df['Adj Close'].rolling(window=high_low_period, min_periods=1, center=False).min()
    ...
    for x in range(starting_index, len(df)):
        ...
        if df["%K"].iloc[x] < 20 and df[f'%D-{d_sma_period}-days'].iloc[x] < 20 and df["%K"].iloc[x] < df[f'%D-{d_sma_period}-days'].iloc[x]:
        ...
        elif df["%K"].iloc[x] > 80 and df[f'%D-{d_sma_period}-days'].iloc[x] > 80 and df["%K"].iloc[x] > df[f'%D-{d_sma_period}-days'].iloc[x]:
...
def backtest(start, end, high_low_period, d_sma_period, dowTickers):
        ...
        if zacetnoObdobje == begining:
            ...
            for x in starting_companies:
                ...
                if x in problematicni and koncnoObdobje == "2008-2-19": # to podjetje ima izjemo
                    ...
                    return_df = stohastic_oscilator(high_low_period, d_sma_period, data, x, 0, 0, True)
...
backtest(start, end, high_low_period, d_sma_period, dowTickers)

"""
test_ticker = "HD"
test_data = yf.download(test_ticker, start=start, end=end, progress=False)
test_data = test_data[['Adj Close']].copy()
test_data = zacetniDf(test_data, sma_period, d_sma_period)  # dodamo stolpce
return_df = bollingerBands(sma_period, bands_multiplayer, test_data, test_ticker, 0, 0, True)

stohastic_oscilator(sma_period, bands_multiplayer, return_df, test_ticker)
"""