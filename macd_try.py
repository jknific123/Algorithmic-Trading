...
def macd(sPeriod, lPeriod,signal_period, df, ticker, starting_index, status, odZacetkaAliNe):
    # naredimo nove stolpce za EMA-e, MACD in signal line

    df[f'EMA-{sPeriod}'] = df['Adj Close'].ewm(span=sPeriod, adjust=False).mean()
    df[f'EMA-{lPeriod}'] = df['Adj Close'].ewm(span=lPeriod, adjust=False).mean()
    df["MACD"] = df[f'EMA-{sPeriod}'] - df[f'EMA-{lPeriod}']
    df[f"Signal-{signal_period}"] = df["MACD"].ewm(span=lPeriod, adjust=False).mean()
    ...
    for x in range(starting_index, len(df)):
        ...
        if df["MACD"].iloc[x] > df[f"Signal-{signal_period}"].iloc[x]:
        ...
        elif df["MACD"].iloc[x] < df[f"Signal-{signal_period}"].iloc[x]:
...
def backtest(start, end, short_period, long_period, signal_period, dowTickers):
        ...
        if zacetnoObdobje == begining:
            ...
            for x in starting_companies:
                ...
                if x in problematicni and koncnoObdobje == "2008-2-19": # to podjetje ima izjemo
                    ...
                    return_df = macd(short_period, long_period, signal_period, data, x, 0, 0, True)
...
backtest(start, end, short_period, long_period, signal_period, dowTickers)
