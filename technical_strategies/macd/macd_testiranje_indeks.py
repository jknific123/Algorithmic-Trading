...
import numpy as np
from technical_strategies.macd.macd_backtester import backtest
        ...
        for ema_long in ema_long_vrednosti:
            for signal in signal_vrednosti:
                if ema_short != ema_long:
                    testni_rezultati[f"[{ema_short},{ema_long},{signal}]"] = {}
                    data_df = data.copy(deep=True)
                    print(f"Kombinacija: Ema_short = {ema_short} , Ema_long = {ema_long} , Signal = {signal}")
                    testni_rezultati[f"[{ema_short},{ema_long},{signal}]"] = macd(ema_short, ema_long, signal, data_df, ticker, 0, 0, True, hold_obdobje, True)
...
def testirajNaIndeksuUcnaMnozica(company_ticker, hold_obdobje, stockPricesDBIndex):
    ...
    with open(f'D:\Faks\Algorithmic-Trading\/technical_strategies\macd\macd_rezultati_indeks_ucna\MACD{company_ticker}_{hold_obdobje_string}.txt', 'w', encoding='UTF8') as f:
...
def testirajOptimalneNaTestniMnoziciZaHoldObdobjaIndeks(testnaStockPricesDB, hold_obdobja_list):
    optimalni_dnevni = [[20, 40, 9], [18, 40, 9], [20, 30, 9]]
    optimalni_tedenski = [[20, 24, 6], [20, 30, 6], [15, 35, 9]]
    optimalni_mesecni = [[18, 40, 6], [12, 35, 9], [12, 30, 9]]
    optimalni_letni = [[20, 40, 3], [18, 40, 3], [20, 35, 3]]
    ...
    for hold_cas in hold_obdobja_list:
        ...
        for kombinacija in trenutni_parametri_list:
            ...
            temp = macd(kombinacija[0], kombinacija[1], kombinacija[2], data_df, '^DJI', 0, 0, True, hold_cas, True)
        ...
        with open(f'D:\Faks\Algorithmic-Trading\/technical_strategies\macd\macd_rezultati_indeks_testna\MACD_^DJI_{hold_obdobje_string}_testna.txt', 'w', encoding='UTF8') as f:
...
def testirajNaIndeksuEnoKombinacijo(start_date, end_date, short_ema, long_ema, signal_period, stock_prices_db, hold_obdobje_kombinacija_indeks):
...
""" Testrianje MACD na DJIA indeksu """
...
print('Testrianje MACD indeks, inicializacija objektov')
...
testirajNaIndeksuEnoKombinacijo(start_date="2005-02-07", end_date="2021-11-21", short_ema=20, long_ema=40, signal_period=3,
