...
from mixed_strategies.moja_mesana.moja_mesana_value.moja_mesana_value_investing_grafi import profit_graph
...
def zacetniDf(data):
    ...
    data['Total'] = 0
    data['Ticker'] = ""
    data['Buy-Signal-Fundamental'] = np.nan
    data['Sell-Signal-Fundamental'] = np.nan
    data['Buy-Signal-Technical'] = np.nan
    data['Sell-Signal-Technical'] = np.nan
    data["Buy-date"] = ""
    data["Sell-date"] = ""
...
def backtest(start, end, sma_period_short, sma_period_long, dowTickers, stockPricesDB, fundamental_data):
    # nastavimo obdobja
    ...
    for i in range(0, len(obdobja) - 1):  # gremo cez vsa obdobja in jih imamo po parih startDATE -> endDATE
        ...
        if zacetnoObdobje == start:
            ...
            for x in starting_companies:
                ...
                if x == "GM":
                    ...
                    prazen["Close"] = 0
                    return_df = value_investing_strategy(start_date=zacetnoObdobje, end_date=koncnoObdobje, sPeriod=sma_period_short, lPeriod=sma_period_long, df=prazen, ticker=x, starting_index=0, status=0, odZacetkaAliNe=True,
                                                         fundamental_data=fundamental_data, potrebnoRezatiGledeNaDatum=True)
                    ...
                    return_df = value_investing_strategy(start_date=zacetnoObdobje, end_date=koncnoObdobje, sPeriod=sma_period_short, lPeriod=sma_period_long, df=data, ticker=x, starting_index=0, status=0, odZacetkaAliNe=True,
                                                         fundamental_data=fundamental_data, potrebnoRezatiGledeNaDatum=True)
                    portfolio[x] = return_df
        ...
        # ce nismo na zacetku gremo cez removed in added in naredimo menjave ter trejdamo za naslednje obdobje
            ...
            for odstranjenTicker in dowTickers[zacetnoObdobje]["removed"]:
                ...
                if odstranjenTicker in starting_companies:  # odstranjenTicker je v trenutnem portfoliu -> ga zamenjamo z isto ležečim tickerjem iz added
                    ...
                    if ex_df["Shares"][-1] == 0:  # super samo prepisemo kes
                        ...
                        ex_df['Sell-Signal-Technical'].to_numpy()[-1] = ex_df["Close"].to_numpy()[-1]
                    ...
                    new_returns = value_investing_strategy(start_date=zacetnoObdobje, end_date=koncnoObdobje, sPeriod=sma_period_short, lPeriod=sma_period_long, df=new_df, ticker=nov_ticker, starting_index=starting_index, status=0,
            ...
            for ostaliTicker in ostali:
                ...
                new_ostaliTickerDataframe = value_investing_strategy(start_date=zacetnoObdobje, end_date=koncnoObdobje, sPeriod=sma_period_short, lPeriod=sma_period_long, df=concat_data, ticker=ostaliTicker, starting_index=starting_index,
                                                                     status=zadnji_signal, odZacetkaAliNe=False, fundamental_data=fundamental_data, potrebnoRezatiGledeNaDatum=False)
