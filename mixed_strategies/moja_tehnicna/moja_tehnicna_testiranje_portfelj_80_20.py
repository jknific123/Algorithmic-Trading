...
def testirajNaPortfoliuUcnaMnozica(dowTickers, stock_prices_db, hold_cas):
    ...
    return_ucni = najdiOptimalneParametreNaPotrfoliu(start_period="2005-11-21", end_period="2018-09-09", dowTickers=dowTickers, stock_prices_db=stock_prices_db,
    ...
    with open(f'D:\Faks\Algorithmic-Trading\/mixed_strategies\/moja_tehnicna\/moja_tehnicna_rezultati_ucna_80_20\/Moja_tehnicna_{hold_obdobje_string}.txt', 'w', encoding='UTF8') as f:
        for x in sorted_rez_total_ucni:
            print(x, ": ", sorted_rez_total_ucni[x])
            row_string = str(x) + ': ' + str(round(sorted_rez_total_ucni[x], 2)) + ' ' + str(
                util.povprecnaLetnaObrestnaMera(rez_zacetna_investicija_ucni[x], sorted_rez_total_ucni[x], util.ucnaMnozicaLeta80procentov())) + '% ' + str(
...
def testirajOptimalneNaTestniMnoziciZaHoldObdobja(dowTickers, testnaStockPricesDB, hold_obdobja_list):
    optimalni_dnevni = [[[15, 14], [20, 2]],  [[9, 8], [10, 1.9]],  [[15, 14], [50, 2.1]]]
    optimalni_tedenski = [[[15, 14], [20, 2]],  [[9, 8], [10, 1.9]],  [[15, 14], [50, 2.1]]]
    optimalni_mesecni = [[[15, 14], [20, 2]],  [[9, 8], [10, 1.9]],  [[15, 14], [50, 2.1]]]
    optimalni_letni = [[[5, 3], [20, 2]],  [[15, 14], [50, 2.1]],  [[15, 11], [50, 2.1]]]
    ...
    for hold_cas in hold_obdobja_list:
        ...
        for kombinacija in trenutni_parametri_list:
            ...
            temp = backtest('2018-09-09', '2021-11-21', short_period_macd=1, long_period_macd=1, signal_period_macd=1,
        ...
        with open(f'D:\Faks\Algorithmic-Trading\/mixed_strategies\/moja_tehnicna\/moja_tehnicna_rezultati_testna_80_20\/Moja_tehnicna_{hold_obdobje_string}_testna.txt', 'w', encoding='UTF8') as f:
            for x in sorted_rez_total_testni:
                print(x, ': ', sorted_rez_total_testni[x], ' ', util.testnaMnozicaLeta20procentov())
                row_string = str(x) + ': ' + str(round(sorted_rez_total_testni[x], 2)) + ' ' + str(
                    util.povprecnaLetnaObrestnaMera(rez_zacetna_investicija_testni[x], sorted_rez_total_testni[x], util.testnaMnozicaLeta20procentov())) + '% ' + str(
...
print('Moja tehnicna strategy portfelj, inicializacije objekta 80% - 20%')
