...
def value_investing_strategy(start_date, end_date, df, ticker, starting_index, status, odZacetkaAliNe, fundamental_data):
    ...
    for x in range(starting_index, len(df)):
        ...
        # manjka -> Sell signal
        elif isDatumOk and check == 2 and pogojSell(currCompany_data=company_data, currCompanyIndustry_data=year_avg_data, datum=trenutni_datum, ticker=ticker, fDB=fundamental_data):
